...
import SchedaVisuale from "../components/SchedaVisuale"
...
export default function SchedaCliente() {
  ...
  async function salvaModifiche() {
    ...
    const { error } = await supabase.from("clienti").update({ nome, cognome, email, telefono, peso_iniziale, altezza, obiettivo, note }).eq("id", id)
    if (error) { console.error("ERRORE:", JSON.stringify(error)); alert("Errore: " + error.message); setSaving(false); return }
    setEditing(false)
    fetchDati()
  ...
  async function generaScheda() {
    ...
    try {
      ...
      if (data.content?.[0]?.text) setSchedaAI(data.content[0].text.replace(/\*\*/g,'').replace(/^###\s*/gm,'').replace(/^##\s*/gm,'').replace(/^#\s*/gm,'').replace(/^---$/gm,''))
      ...
      {tab === "alimentazione" && (
        <div className="card" style={{padding:"1.25rem"}}>
          ...
          {!pianoAI ? (
            ...
            <div>
              <PianoVisuale piano={pianoAI} onModifica={setPianoAI} />
